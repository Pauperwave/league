...
- **Wave 4 is fused with the Pinia Colada migration (ADR-015):** each remaining CRUD entity migrates as one slice — BFF endpoints + reads to `useQuery` + writes to `useMutation` + delete the entity's store — so the store layer is written once, in its final form. **Leagues done 2026-07-17** (template: `server/api/leagues/*`, `app/composables/league/useLeaguesQuery|useLeagueMutations|useLeaguesPage`). Remaining slices: rulesets, players, commander-decks, events-CRUD. The flip prerequisite is unchanged: `SUPABASE_SERVICE_KEY` in every environment, then deny anon writes table by table.
