...
- **Slice progress:** waves 1+2 complete 2026-07-14 — event lifecycle (`register-player`, `unregister-player`, `start`, `advance-round`, `turn-back-round`) and round data (`pairings/:id/rankings|kills|commander|votes`, shared scaffolding in `server/utils/roundResults.ts`). **No client code writes `standings`, `pairings`, `round_results`, or `waitroom` anymore** — all four ready for the anon-write flip once `SUPABASE_SERVICE_KEY` is configured and endpoints switch to `serverSupabaseServiceRole`. Remaining client writes: entity CRUD only (`events`/`players`/`leagues`/`rulesets`/`commander_decks` create/update/delete — wave 4, security-only value).
- **Session hardening 2026-07-17 (ADR-014):** the static `site-auth` cookie is replaced by sealed httpOnly sessions (`nuxt-auth-utils`), checked centrally in `server/middleware/api-auth.ts` for every `/api/**` endpoint — the API gate is no longer forgeable, so the anon-write flip now has a real lock to stand behind. Remaining exposure: direct Data API writes with the extracted anon key (unchanged until the flip). `NUXT_SESSION_PASSWORD` must exist in every deploy environment.
- **Wave 4 is fused with the Pinia Colada migration (ADR-015):** each remaining CRUD entity migrates as one slice — BFF endpoints + reads to `useQuery` + writes to `useMutation` + delete the entity's store — so the store layer is written once, in its final form. **Leagues done 2026-07-17** (template: `server/api/leagues/*`, `app/composables/league/useLeaguesQuery|useLeagueMutations|useLeaguesPage`); **rulesets done 2026-07-18** (in-use delete guard moved server-side → 409); **players/decks/events-CRUD done 2026-07-18 as thin-client conversions** (endpoints + store writes via `$fetch`, reads still store-based — see ADR-015 for why). **Write security is COMPLETE: no client code writes any app table anymore** — every table is ready for the anon-write flip once `SUPABASE_SERVICE_KEY` is configured. Remaining Colada read-side migrations: commander-decks first (self-contained), then players+events together with the event-domain migration (after E2E, BACKLOG #1). The flip prerequisite is unchanged: `SUPABASE_SERVICE_KEY` in every environment, then deny anon writes table by table.
...
- **Incident 2026-07-19: an external tool dropped the anon_all_* policies untracked in migrations — and broke reads, not just writes.** An outside "security cleanup" (a Supabase-side AI assistant, not this codebase) dropped every `anon_all_*` policy (`FOR ALL`, `true`/`true`) on all 12 app tables — the write removal was correct (every client write already goes through the BFF), but those blanket policies had never been tracked in `supabase/migrations/` and, on `player_stats`/`deck_stats`, were the **only** anon read policy at all (the tracked migration only grants `SELECT` to `authenticated`, a role this app never uses). Result: the live site's reads broke app-wide, confirmed empirically via direct REST calls returning empty arrays for known-populated tables. Fixed by `supabase/migrations/20260719020000_restore_anon_read_access.sql` (anon `SELECT`-only, idempotent, applied via the SQL editor and verified table-by-table; anon writes correctly still 401/42501). **Lesson: RLS policies can drift outside version control** (dashboard edits, third-party tooling) — the tracked migrations are not guaranteed to reflect the live policy set. Worth an occasional `pg_policies` audit against `supabase/migrations/` once the service-role flip lands, so this can't silently recur.
