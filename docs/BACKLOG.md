...
- **Slice progress:** `register-player`, `advance-round`, `start`, `turn-back-round` all done 2026-07-14 — **wave 1 complete**. No client code writes `standings` or `pairings` anymore; `waitroom` writes remaining client-side: `removeFromWaitingList` only. All endpoints still use the anon key server-side — switch to `serverSupabaseServiceRole` + deny anon writes per table (needs `SUPABASE_SERVICE_KEY` env) to complete; `standings` and `pairings` are ready to flip now. Next slices: `unregister-player`, then the round-data writes (`scores`/`kills`/`commander`/`votes`), then entity CRUD.
