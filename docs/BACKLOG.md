...
- **Slice progress:** `register-player` done 2026-07-14 (`server/api/events/[eventId]/register-player.post.ts` — cookie gate, valibot body, registration-open guard, duplicate skip, server-truth response; store is a thin `$fetch` client). `advance-round` done 2026-07-14 (atomic round transition; scoring helpers moved to `shared/utils/roundScoring.ts`; round-mismatch 409 guard; optimizer stays client-side, confirmed `playerOrder` in the body). All endpoints still use the anon key server-side — switch to `serverSupabaseServiceRole` + deny anon writes per table (needs `SUPABASE_SERVICE_KEY` env) to complete. Next slices: `start` (startEvent), `turn-back-round`, then the round-data writes (`scores`/`kills`/`commander`/`votes`).
