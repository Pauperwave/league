...
| Storage key(s) | One key, `'league-colada-cache'` | One key per caller+scope: `event-session-${tournamentId}`, waiting-list flags per tournament, `winner-checklist-${tournamentId}-${round}` |
...
- `useSessionStorePersistence.ts` (`app/composables/tournament/useSessionStorePersistence.ts`) mirrors the 4 session stores (rankings/kills/votes/commanders — see [`stores.md`](stores.md)) to `localStorage` on every mutation, keyed per tournament, and restores them via each store's `hydrate()` if the round number still matches on mount. Crash insurance for in-progress round entry that hasn't been submitted to the DB yet.
- `useWaitingListFlags.ts` persists the waiting-list paid/companion checkbox flags per tournament.
- `useWinnerChecklist.ts` (BACKLOG #15) persists the "booster consegnato" check-off state per tournament+round — winners themselves are derived live from `rankingsStore`, only the hand-off checkbox is mirrored here.
