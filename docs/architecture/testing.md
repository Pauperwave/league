...
| Colada query/mutation composables (leagues, rulesets, players, decks, tournaments, commanders) | 🟥 | – | 🟨 (leagues, players create, decks create) |
| Tournament orchestration composables (lifecycle, page, modals, submit handlers, URL sync, waitroom) | 🟥 | – | 🟨 (leagues only, not tournaments) |
| Pinia stores (`tournaments.ts` lifecycle + 4 session stores) | 🟥 | – | 🟨 (indirect, via UI only) |
| Small leaf components (`CurrentTime`, `RowActionButton`) | – | 🟨 | – |
| `StandingsCard` | – | 🟨 | – |
| Tournament page shell, modals, pairing UI, waiting-room, ruleset components | – | 🟥 | 🟥 |
| Player/deck components | – | 🟥 | 🟨 (create only) |
| Server API endpoints (23 total) | 🟥 | – | 🟨 (12 of 23, via the league/player/deck/turn-back-round E2E specs) |
| League CRUD (create/edit/delete via real UI) | – | – | 🟩 |
| Player create, deck create (via real UI) | – | – | 🟨 (create only, no edit/delete UI coverage) |
| Tournament CRUD, lifecycle (start/advance-round/turn-back-round), round-result submission | – | – | 🟨 (start/turn-back-round/rankings submission only, via `turn-back-round.e2e.spec.ts` — advance-round and full CRUD/update/delete-guard paths still untested) |
...
| `composables/tournament/useLiveStandings.test.ts` | `buildPosValues`, `cloneStandings`, `calculatePlayerTableScore` (tied ranks, kills, brew/play votes, zero-score), `updateStanding` (victory only on rank 1) |
| `composables/tournament/useSessionStorePersistence.test.ts` | Hydrate-on-round-match, skip-on-mismatch, persist-on-mutation, Map/Set round-trip, reset-on-round-advance |
...
**Gap**: every composable that talks to Supabase or a BFF endpoint has zero unit tests — nothing in `commanders/`, `deck/`, `league/`, `ruleset/`, most of `players/`, `supabase/useStatsQueryBuilder`, `theme/`, `ui/useBreadcrumb|useButtonLogging|useFormModalMeta`, `tables/useTableCalculator|useTableUtils`, and nearly all of `tournament/` except the two files above (`useTournamentLifecycle`, `useTournamentMutations`, `useTournamentPage`, `useTournamentPlayers`, `useTournamentQueries`, `useTournamentSubmitHandlers`, `useTournamentUrl(Sync)`, `useWaitroom`, `usePairingPresets`, `event-pairing/pairingPreferences`). None of the 5 Pinia stores (`tournaments.ts` + 4 session stores) have a dedicated test file — `useSessionStorePersistence.test.ts` only exercises them via hand-built fakes matching their shape, not their real mutation/getter logic.

## Component tests (`test/nuxt/`) — 3 files, all shallow

| File | What's covered |
|------|-----------------|
| `components/tournament/CurrentTime.test.ts` | Renders HH:mm, ticks forward on timer advance, clock icon present |
| `components/standings/StandingsCard.test.ts` | Single case: title + "submitted" badge render when `submittedByPlayerId` is set |
| `components/ui/actions/RowActionButton.test.ts` | "remove" action → correct color/aria-label; `loading`/`disabled` propagation |

**Gap**: this is the thinnest layer by far. Zero component tests for the entire tournament page shell (`TournamentActionBar`, `TournamentStepper`, `RoundTimer`, `TimerControlButton`, `EndedTournamentBadge`), every tournament modal (score/kill/commander/votes/next-round/form), the whole pairing UI (`PairingsCard`, `PairingSettingsModal`, weights/presets/forbidden-pairs sections, kill-flow canvas, all table card/seat/score-grid components, preview grid/toolbar), waiting-room components, and every deck/league/ruleset/player form modal and display component.

## E2E tests (`test/e2e/`) — 4 specs
...
| `player-create.e2e.spec.ts` | Create a player via the real UI (`/players`), asserting on `/api/players/create` and the resulting row in the players table. No update/delete UI coverage — players are edited via a modal reachable from the table's row actions (see `CreatePlayerModal.vue`), and there is no delete endpoint at all (see `api.md`) |
...
| `turn-back-round.e2e.spec.ts` | API-only (Playwright's `request` fixture, no `page` — reuses the same authenticated storageState) reproduction of BACKLOG #11: create a disposable league/event/3 players → register → start → submit real round scores → turn-back-round, asserting the endpoint succeeds and the round's `round_results` are actually gone, not orphaned. First spec of this shape — closer to the "API/integration" tier BACKLOG #1 describes than to a browser E2E flow, just not yet split into its own `test/api/` directory |

**Gap**: tournament creation, the tournament lifecycle (start → advance-round → turn-back-round), round-result submission (rankings/kills/commander/votes modals), waitroom registration, rulesets, and deck/player edit+delete all remain untested end-to-end. See `BACKLOG.md` #1 for the planned next specs.
