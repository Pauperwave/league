...
2. ~~Il tasto "Ottimizza" sembra riproporre l'ordine di iscrizione al primo turno~~ — root-caused e fixato 2026-08-04, insieme a #3 e #4 (stessa causa), **in due parti**:
   - **Parte 1** (bypass): `TablePreviewModal.vue`'s watcher di auto-optimize bypassava del tutto l'ottimizzatore al turno 1 chiamando `randomizeTables()` invece di `runOptimizer()` — sull'idea (sbagliata per una lega ricorrente) che `table3Count` fosse sempre 0 al turno 1. Rimossa la special-case `currentRound === 1`.
   - **Parte 2** (la causa vera, trovata dopo che l'utente ha fatto notare che i valori erano *effettivamente* zero anche dopo la parte 1): `useLeagueTable3CountsQuery` — la query che porta dentro la storia cross-torneo della lega — è async e **non tracciata da nessun flag di loading** usato dal watcher di auto-optimize. Il `loading` generico della pagina (`useTournamentPage.ts`) copre solo `tournamentStore`/waitroom/eventi, non questa query — quindi l'auto-optimize (gated su `[open, loading, playersForScoring.length]`) poteva partire mentre `leagueTable3CountsData` era ancora `undefined`, ottimizzando con `table3Count: 0` per tutti anche quando la lega HA storia reale. Fix: nuovo `leagueTable3CountsLoading` (`isLoading` della query, esposto da `useTournamentPage.ts`) combinato in un `previewLoading` dedicato passato a `TablePreviewModal`'s `:loading`, invece del `loading` generico condiviso con il resto della pagina.
   - **Non riprodotto live in browser** — verificato via lettura del codice; `pnpm test:unit` (220/220) resta verde ma non c'è un test dedicato a nessuna delle due parti (né al watcher, né alla race condition sul loading).
3. ~~L'ordine di registrazione non dovrebbe contare nulla nel pairing del primo turno~~ — vedi #2, stesso fix.
4. ~~Applicare il peso "Rotazione tavoli da 3" anche al primo turno, usando i dati storici~~ — vedi #2: il peso era già applicato uniformemente in scoring: il problema era che l'ottimizzatore non veniva mai invocato al turno 1 (parte 1), e quando ha iniziato a venire invocato spesso vedeva comunque `table3Count: 0` per via della race condition sul loading (parte 2).
...
### 12. ~~Bug: going back to a previous round breaks navigation state — only the UStepper should control "viewing last round results"~~ — fixed 2026-07-22

Fixed: `useEventLifecycle.ts`'s `resetSessionStores()` (already called after every successful `nextRound`/`turnBackRound`/`startEvent` transition) now also calls the new `clearViewedRound` dependency. One place to fix instead of three separate call sites, since `resetSessionStores()` was already the common thread through `confirmEndEvent`, `handlePreviewConfirm`'s advance-round branch, and `confirmCancelRound`.
