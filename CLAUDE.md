...
- **Stores** (`app/stores/`, Pinia Setup API only): only two kinds remain (ADR-015 migrated every CRUD/read domain to Pinia Colada query composables). `useEventStore` (`app/stores/events.ts`) is the event lifecycle state machine (registration → playing → ended) — `currentEvent` + BFF `$fetch` actions + the ADR-007 `save*` seam, no Supabase client, no cache state. *Session stores* (`rankings`, `kills`, `votes`, `commanders`) hold ephemeral per-round UI state (`Map`/`Set`-based) and implement `reset()`. See [`app/stores/CLAUDE.md`](app/stores/CLAUDE.md) before adding or modifying a store.
