...
- **Prefer a centralized fix over touching many files.** Before mechanically editing dozens of call sites to solve a repeated problem, check whether it can be fixed once instead — a test-setup hook, a config option, a shared module. Concrete example: when every component consuming an auto-imported `app/utils/` constant needed it to work inside Vitest component tests too, the fix was registering it once via `config.global.mocks` in `test/setup.ts`, not adding ~55 explicit per-component imports.
- **`@vue/test-utils`'s `config.global.mocks` only patches the Vue render context** (`_ctx.xxx` lookups inside a mounted component's template/script setup) — it does not help a plain `.ts` module that references another auto-imported export directly (e.g. `app/utils/actionButton.ts` using `ICONS` from `app/utils/icons.ts`), since that's a raw JS reference with no Vue component instance involved. Those still need a real explicit `import`.
