...
| File | Read before |
|------|-------------|
| [`app/components/CLAUDE.md`](app/components/CLAUDE.md) | Adding a new component (folder taxonomy, naming, auto-import) |
| [`app/components/ui/CLAUDE.md`](app/components/ui/CLAUDE.md) | Adding a generic button/modal/layout piece — inventory of reusable UI |
...
- **Stores** (`app/stores/`, Pinia Setup API only): only two kinds remain (ADR-015 migrated every CRUD/read domain to Pinia Colada query composables). `useTournamentStore` (`app/stores/tournaments.ts`) is the tournament lifecycle state machine (registration → playing → ended) — `currentTournament` + BFF `$fetch` actions + the ADR-007 `save*` seam, no Supabase client, no cache state. *Session stores* (`rankings`, `kills`, `votes`, `commanders`) hold ephemeral per-round UI state (`Map`/`Set`-based) and implement `reset()`. See [`app/stores/CLAUDE.md`](app/stores/CLAUDE.md) before adding or modifying a store.
- **Composables** (`app/composables/`) wrap store calls in `useAsyncData` for SSR. Notable ones: `tournament/useTournamentPage.ts` orchestrates the entire tournament page (multiple stores + composables), `event-pairing/pairingOptimizer.ts` is the greedy+local-swap pairing algorithm (see below), `tournament/useTournamentUrlSync.ts` persists modal state to URL query params (`docs/architecture/modal-url-sync.md`). See [`app/composables/CLAUDE.md`](app/composables/CLAUDE.md) for auto-import behavior and folder organization before adding one.
...
- **Auth**: a single global site password gate, not per-user auth — the password unlocks a sealed httpOnly session (`nuxt-auth-utils`, ADR-014): `app/middleware/password.global.ts` checks `useUserSession().loggedIn` for page navigation and redirects to `/login`; `server/middleware/api-auth.ts` enforces the same session on every `/api/**` endpoint (except `/api/auth/*` and Nuxt-internal `/api/_*`). `NUXT_SESSION_PASSWORD` (32+ chars, server-only) must exist in every environment.
- **Tournament lifecycle**: state (`registration` / `playing` / `ended`) is derived from `tournament_playing`, `tournament_current_round`, `tournament_registration_open` — not a separate enum column. Full phase-by-phase DB mutations in `docs/architecture/event-flow.md`.
...
- **Version-bump commits are always `chore(release): 🔖 bump version to X.Y.Z`** — exact type, scope, and gitmoji, no variation. `.githooks/commit-msg` only validates the general `<type>(<scope>)?!?: <emoji> <description>` shape (any valid Conventional Commits type + any gitmoji, scope optional) — it does NOT pin `chore` to `(release)` or to 🔖, so a differently-shaped bump commit (e.g. `chore: ⬆️ bump version to X.Y.Z`) passes the hook but breaks the convention `git log` otherwise holds to consistently. Check past bump commits with `git log --oneline | grep "bump version"` if unsure.
...
- **Async action buttons** (decided 2026-07-14, pairs with ADR-013's intent-based writes): a bare `<UButton>` that triggers a store mutation uses **`loading-auto`** — Nuxt UI drives the spinner/disable from the promise returned by `@click`, preventing double-clicks and UI races during the transition:
  ```vue
  <UButton :label="t('tournament.advanceRound')" loading-auto @click="onAdvanceRound" />
  ```
  ```ts
  async function onAdvanceRound() {
    const { success, error } = await tournamentStore.nextRound()
    if (!success) {
      toast.add({ title: t('tournament.advanceRoundError'), description: error, color: 'error' })
      return
    }
    toast.add({ title: t('tournament.advanceRoundSuccess'), color: 'success' })
  }
  ```
...
- **Rely on Nuxt auto-imports for values** (Vue reactivity APIs, `useI18n`, pinia helpers, `@vueuse/core`, own composables, `app/utils/` exports, stores) — **don't add explicit value imports for them.** Tests get the same treatment: `vitest.config.ts` runs `unplugin-auto-import` mirroring Nuxt's presets and dirs (matching KEEP IN SYNC comments in both configs), so plain `@vue/test-utils` mounts compile identically to the app. Decided 2026-07-14, *inverting* the previous "always import explicitly" rule — that rule existed only to work around the test environment, and fixing the environment once beats defeating Nuxt's auto-import feature in ~200 files. Three boundaries to know:
  ...
  - **Top-level `shared/utils/*.ts` files auto-import too** (`roundScoring.ts`, `standingsSort.ts`, `playerSimilarity.ts`, `logger.ts`) — this is Nuxt's own built-in shared-folder feature (confirmed via `.nuxt/types/shared-imports.d.ts`/`nitro-imports.d.ts`, both values and types, in the app AND in `server/api/**`), decided 2026-08-03. `vitest.config.ts`'s `AutoImport.dirs` lists `./shared/utils` to mirror it. **The `shared/utils/types/` subfolder is the one exception that stays fully explicit** — Nuxt doesn't scan it either, so `import type {...} from '#shared/utils/types'` and value imports like `getPairingPlayerIds` need their usual explicit import.
  (Historical: a `config.global.mocks` shim was tried and reverted — patches only template lookups, not module references. `@nuxt/test-utils`' nuxt environment was considered and rejected — boots a Nuxt runtime per test file, slow and historically finicky on Windows.)
- **i18n translated prop defaults**: `defineProps()` defaults are hoisted by the Vue SFC compiler and **cannot reference locally-declared `<script setup>` consts** (e.g. `const { t } = useI18n()` then `label = t('common.cancel')` as a prop default) — throws a `[@vue/compiler-sfc]` compile error, not just a lint/type issue. Instead, leave the prop as plain optional/undefined and resolve the translated fallback in a `computed()`:
  ...
  ```
  `useI18n` is auto-imported (in the app by `@nuxtjs/i18n`, in tests via the `vue-i18n` preset in `vitest.config.ts`'s AutoImport mirror — `vue-i18n` is kept as a direct dependency so that preset resolves). Components using `useI18n()` also need the plugin actually installed when mounted in `test/nuxt/**` — use `createI18nTestPlugin(messages)` from `test/helpers/mocks.ts` in `global.plugins`, passing only the keys that component actually uses.
- `fallow` (`pnpm fallow:*`) is a complexity/duplication/dead-code auditor, not a linter — it's a separate signal from `eslint`/`tsc`. See the "Common Agent Workflow" section of `docs/AGENTS.md` for how to act on its findings; `.fallowrc.json` defines its entry points and exclusions (notably `shared/utils/types/database.ts`, since that file is meant to be regenerated, not hand-maintained — see the note below; `i18n/i18n.config.ts`, which `fallow` can't see is used because Nuxt loads it via a string path in `nuxt.config.ts`'s `i18n.vueI18n` option, not a static import; and `test/e2e/global.setup.ts`, referenced only by `playwright.config.ts`'s `testMatch` regex, same class of string-path reference).
  - **Glob patterns in `.fallowrc.json` (`ignorePatterns`, `duplicates.ignore`, `health.thresholdOverrides[].files`, etc.) can't use literal `[...]` for Nuxt's dynamic-route folder names** (`[leagueId]`, `[id]`, `[deckSlug]`) — the glob engine reads square brackets as a character class, not literal text. Backslash-escaping (`\\[leagueId\\]`) does not work either (silently fails to match, likely a Windows path-separator conflict since `\` is also the native separator). The fix: replace each literal `[`/`]` with a `?` wildcard instead — e.g. `app/pages/league/?leagueId?/tournament/?tournamentId?.vue` — which matches correctly. Always verify a bracket-path glob actually took effect (re-run the relevant `fallow` command and confirm the file dropped out of the report) rather than trusting the config alone.
