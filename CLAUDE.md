...
- **All UI-facing Italian strings live in `i18n/locales/it.json`** (a plain JSON file, lazy-loaded by `@nuxtjs/i18n` — not inline in `i18n/i18n.config.ts`, which now only holds non-message options like `{ legacy: false, locale: 'it' }`). Add new keys there, nested by domain (`league.*`, `event.*`, `store.*`, etc.), then consume with `const { t } = useI18n()` + `t('domain.key')`. `langDir`/`file` in `nuxt.config.ts` resolve relative to `restructureDir` (default `'i18n'`), so the file's real path is `i18n/locales/it.json`, not `<root>/locales/it.json` — verify against the module's own source (`node_modules/@nuxtjs/i18n/dist/module.mjs`) before changing this, not from memory.
  ...
  - DB-persisted values that happen to be human-readable Italian text (e.g. `leagues.status` = `'Programmata'`/`'Attiva'`/`'Terminata'`, matched exactly elsewhere in code) are **data, not translatable UI copy** — leave them as literal strings.
...
`docs/TODO.md` and `docs/PROGRESS.md` have distinct roles — don't blur them:
- **`docs/TODO.md`**: forward-looking, scratch. Planned work, open questions, in-progress migration notes. Fine for it to be messy/verbose while something is actively underway.
- **`docs/PROGRESS.md`**: backward-looking, curated. Once a TODO item is actually done, fold a summary into `PROGRESS.md` (either its own ADR-#### entry for an architectural decision, or a line in the relevant existing section) and remove/trim the corresponding TODO entry rather than leaving it duplicated in both files.
