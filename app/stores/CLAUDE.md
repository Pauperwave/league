...
**CRUD domains are migrating OFF Supabase stores entirely (ADR-015):** reads move to a per-domain Pinia Colada query composable (`app/composables/<domain>/use*Query.ts`), writes to `useMutation` + BFF endpoints, and the entity's store is deleted — no hybrids. `leagues`, `rulesets`, `commander-decks`, and `players` (incl. the waitroom, now `event/useWaitroom.ts`) are fully migrated. The only remaining Supabase stores are `events.ts` (the lifecycle state machine — its writes already go through the BFF via `$fetch`; never reintroduce `supabase.from().insert/update/delete`) and the read-only `player-stats.ts`. Session stores are NOT affected — ephemeral UI state is not server state.
