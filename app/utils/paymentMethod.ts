...
interface PaymentMethodDisplay {
  icon: IconName
  /** 'neutral' isn't one of the 6 app.config.ts semantic tokens, but is a
   * valid Nuxt UI badge/icon color — used for the 'free' method, which has no dedicated semantic meaning. */
  color: SemanticColor | 'neutral'
  /** Static Tailwind class (not built via string interpolation — the JIT
   * scanner can't see interpolated color names). */
  textClass: string
  labelKey: string
}

/** Icon/color/i18n-key for each `PaymentMethod` value — single source shared
 * by StandingsCard, TournamentRegistrationTable, and the league payments
 * page. Module-scope const, can't call `useI18n()`: resolve `t(labelKey)` at
 * the consuming component's own `setup()`. */
