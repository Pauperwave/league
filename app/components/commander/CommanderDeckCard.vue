      ...
      <div class="flex items-center justify-between p-4">
        <div class="flex flex-col gap-1">
          <div v-if="hasCompanion && !isAggregate" class="text-sm text-muted flex items-center gap-1.5">
            <UIcon name="i-lucide-heart" class="size-3.5 text-error" />
            <span>{{ deck.companion_name }}</span>
          </div>
          <div v-if="lenderName && !isAggregate" class="text-sm text-warning flex items-center gap-1.5">
            <UIcon name="i-lucide-hand-helping" class="size-3.5" />
            <span>di {{ lenderName }}</span>
          </div>
        </div>

        <div class="flex items-center gap-2">
          <UButton size="xs" variant="soft" color="primary" icon="i-lucide-bar-chart-3" :to="statsPageUrl">
            {{ isAggregate ? 'Dettagli' : 'Statistiche' }}
          </UButton>
          <UButton size="xs" variant="ghost" icon="i-lucide-external-link" :to="scryfallSearchUrl" target="_blank">
            Scryfall
          </UButton>
        </div>
      </div>
