...
**Colada domains (ADR-015) skip both the store and `useAsyncData`:** entities migrated to Pinia Colada keep reads in a per-domain `use*Query.ts` (`useQuery`, client → Supabase, SSR handled by the module), writes in `use*Mutations.ts` (`useMutation` → BFF `$fetch` + query-key invalidation), and page orchestration in `use*Page.ts`. `league/` is the migrated template — copy it for the next CRUD slice instead of writing a new store + `supabase/` wrapper pair. **A `use*Mutations.ts` file's payload type (`XFormPayload`/`XUpdatePayload`) is the canonical export other files import** — the modal's `defineEmits`, the page/composable handler receiving the emit, and the mutation's own parameter all point at the same `export interface`, never a hand-written duplicate. See `app/components/ui/CLAUDE.md`'s "`defineEmits` payload types" section for the shipped bug this prevents.
