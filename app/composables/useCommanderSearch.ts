    ...
    try {
      ...
      if (options.playerId !== null && options.playerId !== undefined) {
        const { data: usedData, error: usedError } = await supabase
          ...
          .select('commander_1_name, commander_2_name')
          .eq('player_id', options.playerId)

        if (!usedError && usedData) {
          const usedNames = new Set<string>()
          for (const row of usedData) {
            if (row.commander_1_name) usedNames.add(row.commander_1_name)
            if (row.commander_2_name) usedNames.add(row.commander_2_name)
          }
